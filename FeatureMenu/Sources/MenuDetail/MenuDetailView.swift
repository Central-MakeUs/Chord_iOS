...
public struct MenuDetailView: View {
      ...
      ZStack {
        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 16) {
            titleSection(item: viewStore.item)
            profitSummaryCard(item: viewStore.item)
            Divider()
              .background(AppColor.grayscale200)
            marginGradeCard(status: viewStore.item.status)
            recommendedPriceSection
            ingredientsSection(item: viewStore.item)
          }
  ...
  private func titleSection(item: MenuItem) -> some View {
  ...
  private func profitSummaryCard(item: MenuItem) -> some View {
  ...
  private func marginGradeCard(status: MenuStatus) -> some View {
    HStack(spacing: 6) {
      Text("마진등급")
        .font(.pretendardCTA)
        .foregroundColor(AppColor.grayscale700)
  ...
  private func ingredientsSection(item: MenuItem) -> some View {
    VStack(alignment: .leading, spacing: 12) {
      HStack(spacing: 4) {
        Text("재료")
          .font(.pretendardSubTitle)
